...
public:
    ...
    void test_regex_whitespace(){
      TS_ASSERT (matchRegex(makeRegex("^[\n\t\r ]+"), "   \n\r   aa") == 8);
   }
    void test_regex_blockComment(){
      TS_ASSERT(matchRegex(makeRegex("^/\\*([^\\*]|\\*+[^\\*/])*\\*+/"), "/* co*mmen//t */ stuff") == 16);
   }
    void test_regex_lineComment(){
        TS_ASSERT(matchRegex(makeRegex("^//[^\n\r]+"), "// comment \n newline") == 11)
   }
} ;
